...
-- MAGIC <img width="1px" src="https://ppxrzfxige.execute-api.us-west-2.amazonaws.com/v1/analytics?category=lakehouse&org_id=984752964297111&notebook=%2F01-Data-ingestion%2F01.1-DLT-Wind-Turbine-SQL&demo_name=lakehouse-iot-platform&event=VIEW&path=%2F_dbdemos%2Flakehouse%2Flakehouse-iot-platform%2F01-Data-ingestion%2F01.1-DLT-Wind-Turbine-SQL&version=1">
...
-- MAGIC %md
-- MAGIC Your DLT Pipeline has been installed and started for you! Open the <a dbdemos-pipeline-id="dlt-iot-wind-turbine" href="#joblist/pipelines/a5cfbb70-fb95-4999-b9f2-13f5d1914147" target="_blank">IOT Wind Turbine Delta Live Table pipeline</a> to see it in action.<br/>
-- MAGIC
...
-- display(spark.read.json('/Volumes/lgeorge/demos/turbine_raw_landing/turbine'))
-- display(spark.read.json('/Volumes/lgeorge/demos/turbine_raw_landing/historical_turbine_status')) #Historical turbine status analyzed

-- COMMAND ----------

-- DBTITLE 1,Wind Turbine sensor data
-- %python #uncomment to scan the data from the notebook
-- display(spark.read.parquet('/Volumes/lgeorge/demos/turbine_raw_landing/incoming_data'))
...
AS SELECT * FROM cloud_files("/Volumes/lgeorge/demos/turbine_raw_landing/turbine", "json", map("cloudFiles.inferColumnTypes" , "true"))
...
AS SELECT * FROM cloud_files("/Volumes/lgeorge/demos/turbine_raw_landing/incoming_data", "parquet", map("cloudFiles.inferColumnTypes" , "true"))
...
AS SELECT * FROM cloud_files("/Volumes/lgeorge/demos/turbine_raw_landing/historical_turbine_status", "json", map("cloudFiles.inferColumnTypes" , "true"))

-- COMMAND ----------

CREATE STREAMING TABLE parts 
COMMENT "Turbine parts from our manufacturing system"
AS SELECT * FROM cloud_files("/Volumes/lgeorge/demos/turbine_raw_landing/parts", "json", map("cloudFiles.inferColumnTypes" , "true"))
...
-- MAGIC
-- MAGIC ## Conclusion
-- MAGIC Our <a dbdemos-pipeline-id="dlt-iot-wind-turbine" href="#joblist/pipelines/a5cfbb70-fb95-4999-b9f2-13f5d1914147" target="_blank">DLT Data Pipeline</a> is now ready using purely SQL. We have an end 2 end cycle, and our ML model has been integrated seamlessly by our Data Engineering team.
-- MAGIC
-- MAGIC
...
-- MAGIC
-- MAGIC We are now ready to build our <a dbdemos-dashboard-id="turbine-analysis" href="/sql/dashboardsv3/01ef97bc3e8d18dd8a81b2325e11c452">DBSQL Dashboard</a>to track the main KPIs and status of our entire Wind Turbine Farm and build complete <a dbdemos-dashboard-id="turbine-predictive" href="/sql/dashboardsv3/01ef97bc3e8d18dd8a81b2325e11c452">Predictive maintenance DBSQL Dashboard</a>.
-- MAGIC
-- MAGIC
